    ...
    for cart_item in cart_items:
        ...
        keyboard.append(
            [InlineKeyboardButton(
                btn, callback_data=f'prod_edit_{str(cart_item.product.id)}'
                )]
            )
    if not keyboard:
        return False

    keyboard.append([InlineKeyboardButton(btn_do_nothing, 
...
class ProductMenu:
    ...
    def _btn_get_sum_all(self):
        ...
        return f'{self.product.price} * {self.quantity} = {self.sum_all} руб.'
    ...
    def product_ikb(self):
        keyboard = [
            [InlineKeyboardButton(self._btn_get_sum_all, 
            ...
            [InlineKeyboardButton(
                btn_add_to_cart, 
                callback_data=f'update_cart_{str(self.product.id)}'
                )],
            [InlineKeyboardButton(
                btn_checkout_from_cart, 
                callback_data='checkout_from_cart'
                )],
            [InlineKeyboardButton(
                btn_back, 
                callback_data=f'back_to_product_list_{self.product.category.id}'
                )]
            ]
        if self._product_in_cart:
            keyboard[1].append(
                InlineKeyboardButton(
                    btn_delete, 
                    callback_data='delete_product_from_cart_' + \
                                  str(self.product.cart_items[0].id)
                    )
                )
        return InlineKeyboardMarkup(keyboard)
...
class EditProductMenu(ProductMenu):
    ...
    def product_ikb(self):
        keyboard = [
            ...
            [InlineKeyboardButton(btn_checkout, 
                                  callback_data='checkout_from_cart')],

            [InlineKeyboardButton(
                btn_back_to_cart, 
                callback_data=f'back_to_cart_{self.product.id}')]
            ]
        if self._product_in_cart:
            keyboard[1].append(
                InlineKeyboardButton(
...
class Menu:
    ...
    def _get_ikb_list(product_list):
        ...
        for product in product_list:
            ikb_list.append(
                InlineKeyboardButton(
                    f'{product.price} р. - {product.name}', 
        ...
        if screen_num == 1:
            if len(screen_list) > 1:
                menu.append(
                    [InlineKeyboardButton(
                        '>>>', 
                        callback_data=f'nav_{str(screen_num+1)}'
                        )]
                    )
            ...
            menu.append(
                [InlineKeyboardButton('<<<', 
                                      callback_data=f'nav_{screen_num-1}'), 
                 InlineKeyboardButton('>>>', 
                                      callback_data=f'nav_{screen_num+1}')]
                )
